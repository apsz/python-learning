...
def main():
    ...
    pipeline = print_matched(size=True if (opts.output and 'size' in opts.output) else False,
                             date=True if (opts.output and 'date' in opts.output) else False)
    if opts.smaller:
        pipeline = size_matcher(pipeline, smaller=opts.smaller)
    if opts.bigger:
        pipeline = size_matcher(pipeline, bigger=opts.bigger)
    if opts.suffix:
        pipeline = suffix_matcher(pipeline, opts.suffix)
    if opts.days:
        pipeline = date_matcher(pipeline, opts.days)
    pipeline = get_files(pipeline)

    for file_or_dir in args:
        pipeline.send(file_or_dir)
    pipeline.close()
