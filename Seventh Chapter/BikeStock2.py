...
import os
import struct
from . import SimplerBinaryRecordFile
...
class BikeStock:
    ...
    def __init__(self, filename):
        self.__fh = SimplerBinaryRecordFile.BinaryRecordFile(filename,
                                                      _BIKE_STRUCT.size)
    ...
    def __delitem__(self, identity):
        index = self.__identity_to_index[identity]
        del self.__fh[index]
        del self.__identity_to_index[identity]
        for key, value in self.__identity_to_index.items():
            if value > index:
                self.__identity_to_index[key] = value -1
