...
class MainWindow:
    ...
    def __init__(self, parent):
        ...
        for label, command, shortcut_text, shortcut_comb in (
                ...
                ("Import...", self.fileSave, "Ctrl+I", "<Control-i>"),
                ("Export...", self.fileSave, "Ctrl+X", "<Control-x>"),
                (None, None, None, None),
    ...
    def fileImport(self, *ignore):
        ...
        filename = tkinter.filedialog.askopenfilename(
                ...
                initialdir=dir,
                filetypes=[("Bookmarks DBM files", "*.dbm.dat")],
                defaultextension=".dbm.dat", parent=self.parent)
        if not filename:
            return False
        try:
            new_data = shelve.open(filename[:-4], protocol=pickle.HIGHEST_PROTOCOL)
            self.data.update(new_data)
            self.dirty = True
            new_data.close()
            for name in sorted(self.data, key=str.lower):
                self.listBox.insert(tkinter.END, name)
            self.setStatusBar("Loaded {0} bookmarks from {1}".format(self.listBox.size(), filename))
        except (EnvironmentError, pickle.PickleError) as err:
            print(err)
            return False

    def fileExport(self, *ignore):
        filename = tkinter.filedialog.asksaveasfilename(title='Bookmarks - Export File',
                                                        initialdir='.',
                                                        filetypes=[('Bookmarks DBM files', '*.dbm.dat')],
                                                        parent=self.parent)
        if not filename:
        ...
        if not self.filename.endswith('.dbm.dat'):
            self.filename += '.dbm.dat'
        try:
            db = shelve.open(self.filename[:-4], protocol=pickle.HIGHEST_PROTOCOL)
            db.update(self.data)
            db.close()
            self.dirty = False
